  ...
  if (!fs.existsSync(EXPLORER_COMPONENT_PATH)) {
    ...
    if (
      ...
      if (originalMapFn.test(content)) {
        ...
        console.log(
          "⚠️  Could not find mapFn in Explorer source, patch may need manual application",
        )
      }
    }
...
function patchBreadcrumbs() {
  if (!fs.existsSync(BREADCRUMBS_COMPONENT_PATH)) {
    ...
    if (content.includes("stripNumericPrefix(node.displayName)")) {
      ...
      const newCode =
        "const crumb = formatCrumb(stripNumericPrefix(node.displayName), slug, simplifySlug(node.slug));"
      ...
      if (content.includes(oldCode)) {
        ...
        console.log(
          "⚠️  Could not find Breadcrumbs source code pattern, patch may need manual application",
        )
      }
    }
  }
  ...
  const distNewCode =
    "const crumb = formatCrumb(stripNumericPrefix(node.displayName), slug2, simplifySlug(node.slug));"
  ...
  if (distContent.includes(distOldCode)) {
    ...
    console.log(
      "⚠️  Could not find Breadcrumbs dist code pattern, patch may need manual application",
    )
  }
}
...
function patchDarkmode() {
  if (!fs.existsSync(DARKMODE_INLINE_SCRIPT_PATH)) {
    ...
    const oldSource = 'const currentTheme = localStorage.getItem("theme") ?? userPref;'
    const newSource = 'const currentTheme = localStorage.getItem("theme") ?? "dark";'
